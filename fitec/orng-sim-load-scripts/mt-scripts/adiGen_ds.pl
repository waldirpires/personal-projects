...
# xwalrib  1/nov/13 v1.13       adding functionalities for load testing at metrology
# xwalrib  3/dec/13 v1.14       adding functionalities for load testing at metrology for ORNG dataset
...
if ($propertiesHash{adiformatspecific} eq "no"){
...
#overwriting the id number from properties file
$propertiesHash{idnumber} = $idnumber;
...
#if ADI file is NOT specific for each template
if ($propertiesHash{adiformatspecific} =~ /yes/i){
  print "Format NOT specific . . . using general template.\n";
...
} else { # ADI file specific
print "Format specific . . . using specific template.\n";

opendir(DIR, $content) or die $!;

# get file from content folder
  $propertiesHash{template} = "$content/ADI.xml";
  $propertiesHash{token} = "$content/metadata.adi.token"; 
}
...
{
unless( -f "$propertiesHash{template}" )   { die "ERROR: No metadata file found: [$propertiesHash{template}] $!\n"; }
unless( -f "$propertiesHash{token}" ) { die "ERROR: No token file found: [$propertiesHash{token}] $!\n"; }
}
...
print "\n-CLOCK ${id} Prepare Params time ${time}s\n";
...
my $tokenFile = "$propertiesHash{tmp}/metadata.adi.token";
system("cp -v $propertiesHash{token} $tokenFile");
print "Token path = $propertiesHash{token} to $tokenFile\n";

if ($propertiesHash{checksum} eq "yes"){
  $time = gettimeofday;
  getCheckSum("$tokenFile");
  $time = gettimeofday - $time;
  print "${id} Get Checksum time ${time}s\n";  
}
...
unless( $propertiesHash{providerid} =~ /NONE/ ) {
    setProvider("$tokenFile",$propertiesHash{providerid});
}

# Update product, if specified as input parameter.
unless( $propertiesHash{product} =~ /NONE/ ) {
    setProduct("$tokenFile",$propertiesHash{product});
}
...
#########################################################################
    ...
    unless( $propertiesHash{idnumber} =~ /NONE/ ) {
  	  print "sed -i \'s/^\@BATCH-ID\@=.*/\@BATCH-ID\@=$propertiesHash{idnumber}/\' $tokenFile";
	  $cmd = "sed -i \'s/^\@BATCH-ID\@=.*/\@BATCH-ID\@=$propertiesHash{idnumber}/\' $tokenFile";
      system("$cmd");	
	}    
...
$time = gettimeofday - $time;
print "\n-CLOCK ${id} Set Tokens time: ${time}s\n";
...
#for($count = $seed; $count < $seed + $propertiesHash{limit}; $count++ ) {
  ...
  $fileId = ${id};
  #system("$propertiesHash{tools}/subToken.pl -aline='$propertiesHash{template},$propertiesHash{tmp}/ADI.xml' -token=$propertiesHash{token} -seed=${count}");
  system("$propertiesHash{tools}/subToken.pl -aline='$propertiesHash{template},$propertiesHash{tmp}/ADI.xml' -token=$propertiesHash{token} -seed=0");
 print "Template Folder: $propertiesHash{template}, destination = $propertiesHash{tmp}/ADI.xml, token=$propertiesHash{token}\n";

  print "\n= = = = = = = = = = = = = = = =\n";
 
  $time = gettimeofday;
  #$cmd = "sed -i \'s/^\@ID-TOKEN\@=.*/\@ID-TOKEN\@=${id}\/\' $propertiesHash{tmp}/ADI.xml";
  #my $cmd = "sed -i 's/^\@ID-TOKEN@/${id}/' $propertiesHash{tmp}/ADI.xml";
  #system("$cmd");
  setId2("$propertiesHash{tmp}/ADI.xml", "$id");
  ...
  system("ls -la $propertiesHash{tmp}/");
  
  system("mv $propertiesHash{tmp}/ADI.xml $propertiesHash{tmp}/${fileId}_ADI.xml");
  #system("cp -v $propertiesHash{content}/ADI.xml $propertiesHash{tmp}/ADI.xml");
  ...
  $time = gettimeofday - $time;
  print "\n-CLOCK ${fileId} Copy Files to TMP time: ${time}s\n";
  ...
  system(" rm -v $propertiesHash{tmp}/*.token");
  ...
  unless($propertiesHash{ingestfiles} =~ /yes/i)
  {
	  ...
	  $time = gettimeofday - $time;
      print "\n-CLOCK ${fileId} Create TAR time: ${time}s\n";
  ...
  if($propertiesHash{unpack} =~ /yes/i) {
	  ...
	  $time = gettimeofday - $time;
      print "\n-CLOCK ${fileId} UnTAR time: ${time}s\n";	  
  }
  ...
  if($propertiesHash{ingest} =~ /yes/i) { 
	if($propertiesHash{ingestfiles} =~ /yes/i) {#ingest files
	  ...
	  my $outDir = "$propertiesHash{out}/$packageId/";
	 $outDir =~ s/\[/\\[/g;
	 $outDir =~ s/\]/\\]/g;
	  
      system("mv $outDir/* $propertiesHash{ingestdir}/; chown nobody:nobody $propertiesHash{ingestdir}/*");
	  system("rm -rf $outDir");
      $time = gettimeofday - $time;
      print "\n-CLOCK ${fileId} Move Files to CMS WF time: ${time}s\n";
      ...
      $time = gettimeofday - $time;
      print "\n-CLOCK ${fileId} Move TAR to CMS WF time: ${time}s";
      print "\n";
	}
  }
  print "\n= = = = = = = = = = = = = = = =\n";
...
$time = gettimeofday - $time;
print "\n-CLOCK ${fileId} Remove TMP files time: ${time}s";
...
sub getCheckSum {
    ...
    my $cmd;
     ...
     $cmd = "sed -i \'s/^\@PACKAGE-REF01-ID-TOKEN\@=.*/\@PACKAGE-REF01-ID-TOKEN\@=/\' $file";
     system("$cmd");
...
###################################################################################
#  subroutine: getADIfileNameFromDir                                                
###################################################################################

sub getADIfileNameFromDir{
  my $dir = $_[0];
